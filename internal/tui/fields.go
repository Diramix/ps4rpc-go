...
func settingsFields() []field {
	return []field{
		{kind: fieldSection, label: "Connection"},
		{
			...
			get:   func(c *config.Config) string { return c.Var.IP },
		...
		{kind: fieldSection, label: "Rich Presence"},
		{
			...
			getBool: func(c *config.Config) bool { return c.Var.Enabled },
			toggle:  func(c *config.Config) { c.Var.Enabled = !c.Var.Enabled },
		},
		{
			kind:  fieldText,
			label: "Client ID",
			help:  "default Discord application ID",
			get:   func(c *config.Config) string { return strconv.FormatInt(c.Var.ClientID, 10) },
			set: func(c *config.Config, v string) error {
		...
		},
		{
			...
			get:   func(c *config.Config) string { return strconv.Itoa(c.Var.WaitTime) },
			set: func(c *config.Config, v string) error {

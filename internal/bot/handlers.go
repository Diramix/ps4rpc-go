...
func (b *Bot) findTitleByComm(comm string) (ps4info.TrophyTitle, error) {
	titles, err := b.info.TrophyTitles()
	if err != nil {
		return ps4info.TrophyTitle{}, err
	}
	for _, t := range titles {
		if t.CommID == comm {
			return t, nil
	...
	return ps4info.TrophyTitle{}, fmt.Errorf("not found")
}

func findGame(games []ps4info.Game, name string) (ps4info.Game, bool) {

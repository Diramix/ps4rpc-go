...
func TestStatusEmbedGame(t *testing.T) {
	st := ps4info.Status{Online: true, State: ps4info.StateGame, TitleID: "CUSA00512", SessionStart: time.Now().Add(-time.Hour)}
...
func TestTrophyEmbed(t *testing.T) {
	title := ps4info.TrophyTitle{CommID: "NPWR13348_00", Name: "The Last of Us™ Part II", Progress: 9, Unlocked: 2, Total: 28, Platinum: 1, Gold: 7, Silver: 9, Bronze: 11}
	trophies := []ps4info.Trophy{
		{ID: 1, Grade: ps4info.GradeGold, Unlocked: true, Name: "What I Had to Do", Description: "Complete the story", TimeUnlocked: time.Now()},
		{ID: 2, Grade: ps4info.GradeBronze, Unlocked: false, Hidden: true, Name: "Secret", Description: "hidden"},
	}
